...
const addToPlaylist = async (accessToken, playlistId, uris, playlistUrl) => {
	...
	if (data.snapshot_id) {
		console.log(`${playlistUrl}: ${uris.length} succesfully added to playlist.`)
	} else {
...
const processRadioPlaylists = async ({ url, spotifyPlaylistId, extractor }, accessToken) => {
	const tracks = await getTracks(url, extractor);
	const trackUris = await Promise.all(tracks.map(track => findTrack(accessToken, track)));
	const existingTrackUris = await getPlaylistTracks(accessToken, spotifyPlaylistId);
